...
<script>
  import {getCarTypePage,removeCarType} from '../../../api/api' 
  import add from '../../../components/system/type/addType'
  import update from '../../../components/system/type/updateType'
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getCarTypePage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeCarType({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
